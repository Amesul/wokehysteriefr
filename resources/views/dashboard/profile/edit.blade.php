<x-dashboard-layout>
        ...
        @if(isset($user->profile_picture) ? Storage::exists($user->profile_picture) : false)
            <x-slot:image>
                <img src="{{ url(substr_replace($user->profile_picture, 'storage', 0, 6)) }}"
                     class="my-auto mb-2 ml-10 w-16 sm:w-20 rounded-full"
    ...
    <form action="/dashboard/profile" method="post" enctype="multipart/form-data">
        ...
        <section class="mx-auto mt-8 rounded-xl bg-white px-6 py-10 shadow-xl sm:mt-12 lg:mt-16 md:p-12">
            <main class="mb-10 grid md:grid-cols-2 gap-x-16 gap-y-10">
                {{-- Display name --}}
                <div>
                    <x-forms.input-label required="true" for="display_name" :value="__('Nom d\'affichage')"/>
                ...
                {{-- Username --}}
                <div>
                    <x-forms.input-label required="true" for="username" :value="__('Nom d\'utilisateur')"/>
                ...
                {{-- Email --}}
                <div>
                    <x-forms.input-label required="true" for="email" :value="__('Email')"/>
                ...
                {{-- Profile picture --}}
                <div>
                    <x-forms.input-label required="true" for="profile_picture" :value="__('Photo de profil')"/>
        ...
        <section class="mx-auto mt-8 rounded-xl bg-white px-6 py-10 shadow-xl sm:mt-12 lg:mt-16 md:p-12">
            <main class="mb-10 grid md:grid-cols-2 gap-x-16 gap-y-10">
                ...
                <div class="col-span-2">
                    <x-forms.input-label for="biography" class="mb-2" :value="__('Biographie')"/>
                ...
                @if($user->team)
                    {{-- Job --}}
                    <div class="col-span-2 sm:col-span-1">
                        <x-forms.input-label for="job" :value="__('Role')"/>
                        <x-forms.form-input id="job" name="job" class="mt-1 block w-full"
                                            :value="old('job', $user->job)"
                                            autofocus/>
                        <x-forms.input-error class="mt-2" :messages="$errors->get('job')"/>
                    </div>
                @endif
            </main>
            <x-forms.primary-button class="ml-auto">Enregistrer</x-forms.primary-button>
    ...
    <section class="mx-auto mt-8 rounded-xl bg-white px-6 py-10 shadow-xl sm:mt-12 lg:mt-16 md:p-12">
        <form action="/dashboard/profile/password" method="post">
            ...
            <main class="mb-10 grid md:grid-cols-2 gap-x-16 gap-y-10">
                {{-- Current password --}}
                <div>
                    <x-forms.input-label required="true" for="current_password" :value="__('Ancien mot de passe')"/>
                    <x-forms.form-input id="current_password" name="current_password" type="current_password"
                ...
                {{-- New password --}}
                <div>
                    <x-forms.input-label required="true" for="new_password" :value="__('Nouveau mot de passe')"/>
                ...
                {{-- Confirmation --}}
                <div>
                    <x-forms.input-label required="true" for="new_password_confirmation" :value="__('Confirmation')"/>
