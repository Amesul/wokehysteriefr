...
<article
    x-data="{open: false}"
    @click="open = true"
    {{ $attributes->class(['mx-auto mt-7 cursor-pointer rounded-xl bg-white px-6 py-4 text-lg shadow-md outline-dashed outline-2 transition-all duration-300 outline-blue-violet group hover:bg-blue-violet hover:shadow-2xl hover:outline-offset-4 hover:outline-black']) }}>
    <h5 class="text-center font-bold transition-all duration-300 group-hover:text-white">Ajouter un
        membre</h5>
    <i class="mx-auto block text-center text-3xl transition-all duration-300 fa-solid fa-circle-plus group-hover:text-white"></i>
    <div x-show="open"
